/**
 * \file src/git.rs
 * cargo-generate is Licensed under the Apache License, Version 2.0 (the "License")
 *
...
pub fn create(project_dir: &Path, config: GitConfig) -> Result<String> {
    ...
    fn check_dir_all(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> Result<()> {
        ...
        for src_entry in fs::read_dir(src)? {
            ...
            if entry_type.is_dir() {
                ...
                warn!("{}", style("Symbolic links not supported").bold().red(),)
            }
        }
        Ok(())
    }
    ...
    if git_dir.exists() && git_dir.is_dir() {
        if let Err(e) = remove_dir_all(git_dir) {
            ...
            if e.to_string().contains(
                ...
                if attempt == 5 {
                    warn!("megflow-quickstart was not able to delete the git history after {} retries. Please delete the `.git` sub-folder manually", attempt);
                    return Ok(());
                }
...
mod tests {
    ...
    const REPO_URL: &str = "https://github.com/MegEngine/MegFlow.git";
    const REPO_URL_SSH: &str = "git@github.com:MegEngine/MegFlow.git";
    ...
    fn should_determine_repo_kind() {
        for (u, k) in &[
            ...
            (
                "http://github.com/MegEngine/MegFlow.git",
                RepoKind::RemoteHttp,
            ),
    ...
    fn should_support_abbreviated_repository_short_urls_like() {
        assert_eq!(
            GitConfig::new_abbr("MegEngine/MegFlow".into(), None, None)
                .unwrap()
                .remote
                .as_ref(),
